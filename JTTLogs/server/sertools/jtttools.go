...
var (
	StreamInfo sync.Map			//实时视频
	StreamChannel sync.Map		//实时视频
	VideoReply sync.Map			//录像
	VideoList sync.Map			//录像列表
	GPSInfoListM sync.Map		//gps列表
	AlarmInfoListM sync.Map		//报警列表
	RealGPSInfo sync.Map		//实时gps
	NewGPSInfo sync.Map			//最新设备上传gps
	NormalResInfo sync.Map		//通用回复存储
	ChangeToTCP sync.Map		//链接切换至tcp
	ChangeToUDP	sync.Map		//链接切换至udp
)
...
func init() {
	udpConnCache = cache.New(60*time.Second, 15*time.Second)
...
//使用PhoneNum删除指定的UDPClient
func DeleteUDPClient(PhoneNum string) {
	udpConnCache.Delete(PhoneNum)
	logs.BeeLogger.Info("deviceID=%s for udp is delete", PhoneNum)
	return
}
...
func init() {
	tcpConnCache = cache.New(60*time.Second, 15*time.Second)
}
...
//使用PhoneNum删除指定的TCPClient
func DeleteTCPClient(PhoneNum string) {
	tcpConnCache.Delete(PhoneNum)
	logs.BeeLogger.Info("deviceID=%s for tcp is delete", PhoneNum)
	return
}
...
//处理消息体属性
func HandleTCPMessProperty(messByte []byte) (int64, bool) {
